...
class JohnAdminScreen extends StatefulWidget {
  const JohnAdminScreen({super.key});

  @override
  State<JohnAdminScreen> createState() => _JohnAdminScreenState();
}

class _JohnAdminScreenState extends State<JohnAdminScreen> {
  ...
  DocumentReference<Map<String, dynamic>> get _configRef =>
      _db.collection('ai_hostess').doc('john');
  ...
  void _applyFromDoc(Map<String, dynamic> data) {
    ...
    _personaName.text = (data['personaName'] as String?) ?? 'John';
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            tr(
              context,
              en: 'John settings saved.',
              es: 'Configuración de John guardada.',
            ),
          ),
        ),
      );
  ...
  Future<void> _uploadMedia({required bool video}) async {
    ...
    final filename =
        'ai_hostess/john/${DateTime.now().millisecondsSinceEpoch}.$ext';
    ...
    final title = tr(
      context,
      en: video ? 'John Video' : 'John Image',
      es: video ? 'Video de John' : 'Imagen de John',
    );
    setState(() {
      ...
      builder: (context, snapshot) {
        ...
        return ValueListenableBuilder<String>(
          ...
          builder: (_, code, __) {
            final isEs = code == 'es';
            final title =
                isEs ? 'Panel de John' : 'John Control Panel';
            final denied = isEs
                ? 'Acceso denegado.'
                : 'Access denied.';
            ...
            final onlineLabel =
                isEs ? 'AI en línea activado' : 'Online AI enabled';
            final offlineLabel =
            ...
            final personaEnLabel =
                isEs ? 'Persona (Inglés)' : 'Persona (English)';
            final personaEsLabel =
                isEs ? 'Persona (Español)' : 'Persona (Spanish)';
            final greetEnLabel =
                isEs ? 'Saludo (Inglés)' : 'Greeting (English)';
            final greetEsLabel =
                isEs ? 'Saludo (Español)' : 'Greeting (Spanish)';
            final voiceEnLabel =
                isEs ? 'Voz (Inglés)' : 'Voice hint (English)';
            final voiceEsLabel =
                isEs ? 'Voz (Español)' : 'Voice hint (Spanish)';
            ...
            return Scaffold(
              ...
              body: ListView(
                ...
                children: [
                   _LinkTile(
                    title: isEs ? 'Fotos de Perfil (John)' : 'Profile Pictures (John)',
                    subtitle: isEs
                        ? 'Gestiona avatares y rotación.'
                        : 'Manage avatars and rotation.',
                    icon: Icons.face,
                    onTap: () => Navigator.pushNamed(context, '/john-profile'),
                  ),
  ...
  Widget _knowledgeCard(int index, _KnowledgeEntry item) {
    ...
    final titleLabel = isEs ? 'Título' : 'Title';
    final keywordLabel = isEs
        ? 'Palabras clave (separadas por coma)'
        : 'Keywords (comma separated)';
    final contentEnLabel =
        isEs ? 'Contenido (Inglés)' : 'Content (English)';
    final contentEsLabel =
        isEs ? 'Contenido (Español)' : 'Content (Spanish)';
    return Card(
  ...
  _AiConfig _defaultConfig() {
    return _AiConfig(
      ...
      personaName: 'John',
      personaBioEn:
          'You are John, the suave, worldly, and helpful concierge of Niña Verde. '
          'You are professional, yet approachable, and focused on providing elite service.',
      personaBioEs:
          'Eres John, el anfitrión suave, cosmopolita y servicial de Niña Verde. '
          'Eres profesional pero accesible, enfocado en brindar un servicio de élite.',
      greetingEn:
          'Good day. I am John. How may I assist you with your Niña Verde experience today?',
      greetingEs:
             'Buen día. Soy John. ¿Cómo puedo asistirle con su experiencia en Niña Verde hoy?',
      voiceEn: 'en-US, professional, warm',
      voiceEs: 'es-NI, professional, warm',
      knowledge: [],
      media: [],
    );
  }
...
class _MediaEntry {
  ...
  factory _MediaEntry.fromMap(Map<String, dynamic> data) {
    return _MediaEntry(
      title: data['title'] as String? ?? 'John Media',
